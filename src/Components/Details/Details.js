...
class ConnectedDetails extends Component {
  ...
  async fetchProductUsingID(id) {
    ...
    axios.get("http://localhost/pelanggaran_sekolah/public/produk").then(response => {
      ...
      var datId =  res.length === 0 ? null : res[0];
      ...
      if (this.isCompMounted) {
        this.setState({
          item:datId,
          relatedItems: res.filter(x => x.id !==res.id),
          loading: false,
        });
      }
    ...
    return (
      <div style={{ padding: 10 }}>
        ...
        <div style={{ display: "flex" }}>
          <img src={"http://127.0.0.1/pelanggaran_sekolah/public/images/produk/"+this.state.item.image_produk} alt="" width={250} height={250} style={{ borderRadius: "5%", objectFit: "cover" }} />
          <div
            style={{
              flex: 1,
              marginLeft: 20,
          ...
          >
            ...
            <Button
              ...
              onClick={() => {
                ...
                const produkCheck = [{"id":this.state.item.id,"name":this.state.item.name,"quantity":this.state.quantity,"price":this.state.item.price}]

                localStorage.setItem("item_produk",JSON.stringify(produkCheck))
